...
class AboutTraits extends KoanSuite {
  koan(
    "A class can use the \'extends\' keyword to mixin a trait if it is the only relationship the class inherits"
  ) {
      ...
      def listen(event: Event): Unit = {
        event match {
          case Event("Moose Stampede", _) =>
            println("An unfortunate moose stampede occured")
  ...
  koan(
    """A class can only \'extends\' from _one_ class or trait, any subsequent extension
      ...
      def listen(event: Event): Unit = {
        event match {
          case Event("Woodchuck Stampede", _) =>
            println("An unfortunate woodchuck stampede occured")
          case _ => println("Nothing of importance occured")
        }
      }
    }
  ...
  koan(
    """Protip: For many; using \'extends\' gives the impression of a \'is-a\' relationship and many don't want to
  ...
  koan(
    "Traits are polymorphic. Any type can be referred to by another type if related by extension or trait"
  ) {
  ...
  koan(
    """Traits can have concrete implementations, but can still be overridden by classes that
          |  mixin the trait"""
  ) {
    ...
    val welder = new Welder
    welder
      .weld() // Parenthesis are used to make calls on methods with side-effects

    val baker = new Baker
    baker
      .bake() // Parenthesis are used to make calls on methods with side-effects
  ...
  koan(
    """Traits can be stacked with other traits to create customizable decorative abstractions for a class"""
  ) {
  ...
  koan(
    """Traits can be stacked with other traits to create customizable decorative
    ...
    val baker =
      new Baker
        with ListLog
        with TimedLog // Pick and choose what traits to stack!
        with UserLog

    baker.log("baked cake")
    baker.logCache.head should be(
      "January, 12, 2025 : Root said: baked cake"
    ) // Whoa!
  }
  ...
  koan(
    "Traits are instantiated before a classes instantition from left to right"
  ) {
    var sb = List[String]()
  ...
  koan(
    """Instantiations are tracked and will not allow a duplicate instantiation.
  ...
  koan(
    "The diamond of death (http://en.wikipedia.org/wiki/Diamond_problem) is avoided since " +
