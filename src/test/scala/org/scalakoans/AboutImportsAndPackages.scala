...
package subpackage {
  ...
  class AboutImportsAndPackagesInSubpackages extends KoanSuite {
    koan(
      "A package can be included in a file with an established established package, " +
...
package media {
  ...
  class AboutReferencingAbsolutePackages extends KoanSuite {
    ...
    koan("A import can be done based from absolute package heirarchy") {
      val stLouisBlues =
        new Album("St. Louis Blues", 1940, new Artist("Louie", "Armstrong"))
      stLouisBlues.getClass.getCanonicalName should be("java.lang.album")
    }
  }
  ...
  class AboutReferencingAbsoluteRootPackages extends KoanSuite {
    ...
    // <<< Note the import style
    koan(
      "A import can be done based from absolute root package heirarchy using _root_"
    ) {
      val stLouisBlues =
        new Album("St. Louis Blues", 1940, new Artist("Louie", "Armstrong"))
      stLouisBlues.getClass.getCanonicalName should be(
        "_root_.org.scalakoans.album.Album"
      )
    }
  }
...
class AboutImportingTechniques extends KoanSuite {
  ...
  koan(
    "To import all classes of a package, use can also use {_} as a wildcard"
  ) {
    import music_additions.{_}
    val genre = new Genre("Jazz")
    val producer = new Producer("Joe", "Oliver")
  ...
  koan(
    "To import a select group of classes of a package, use {className1, className}"
  ) {
  ...
  koan(
    "You can rename a class by using =>, and also import all other classes in a package keeping their name"
  ) {
  ...
  koan(
    "You can just import the package themselves,so you can give it a verbose identity"
  ) {
    import scala.collection.mutable
    val arrayBuffer =
      mutable.ArrayBuffer.range(2, 10) // sounds better: A Mutable ArrayBuffer
