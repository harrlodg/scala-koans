...
class AboutHigherOrderFunctions extends KoanSuite {
  ...
  koan("A variable referencing an anonymous function") {
    ...
    def result = List(
      ...
      2,
      3
    ) map lambda // FYI: map runs a function on each element of a collection
  ...
  koan(
    """Another way for a variable to reference an explicitly created function.  In this case (Int) => Int
  ...
  koan(
    "function taking another function as parameter. Helps in compositioning functions"
  ) {
  ...
  koan(
    "Currying is a technique to transform function with multiple parameters to function with one parameter"
  ) {
  ...
  koan("Using a method inside a class to create a function") {
    ...
    assert(List(12, 11, 44) === List(12, 2, 3, 4, 5, 6, 11, 44).filter(x))
    // ???
    // surely it makes more sense to return the values where the amount is less than
    // the capacity?
  }
}
